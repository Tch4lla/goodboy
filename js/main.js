...
fetch(BREEDS_URL)
  ...
  .then(data => {
    ...
    for(let i = 0; i < breedsArray.length; i++){
      ...
      option.innerText = breedsArray[i] //readable text for each value
      SELECT.appendChild(option)//adding each option to the dropdown
    }
  })
...
const doggoInfo = {
  ...
  assignFactList(){
    this.factList = this.yatesShuffle(this.factList)
    document.getElementById('fun-fact').innerHTML = `Additional Info: ${this.factList[0]}`
  }
